...
const CategoryRepository = {
    ...
    getAll: async (req) => {
        try {
            ...
            var aggregate = CategoryModel.aggregate([
                {
                    $project: {
                        _id: "$_id",
                        category: "$category",
            ...
            let allRecord = await CategoryModel.aggregatePaginate(aggregate, options);
    ...
    getById: async (id) => {
        ...
        try {
            let record = await CategoryModel.findById(id).lean().exec();
    ...
    getByField: async (params) => {
        ...
        try {
            let record = await CategoryModel.findOne(params).exec();
    ...
    updateById: async (id, data) => {
        try {
            let record = await CategoryModel.findByIdAndUpdate(id, data, {
                new: true
            });
            if (!record) {
                return null;
